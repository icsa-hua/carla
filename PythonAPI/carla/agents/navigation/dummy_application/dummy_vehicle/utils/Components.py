...
class PCI(Design): 
    ...
    def initialize_performance_parameters(self)->None:
        self.torque_curve = [self.to_serialize(vector) for vector in self.pci.torque_curve]
        self.steering_curve = [self.to_serialize(vector) for vector in self.pci.steering_curve]


    def objectify_performance_parameters(self) -> None:
        self.torque_curve = self.from_serialize(self.pci["torque_curve"])
        self.torque_curve = [torque for torque in self.torque_curve]
        self.steering_curve = self.from_serialize(self.pci["steering_curve"])
    ...
    def get_wheels(self, wheels)->list: 
        wheels = []
        for wheel in wheels: 
            wheels.append(self.serialize_wheel(wheel))
        return wheels
