#Autonomoys vehicle and obstacle recognition and PFA with A*. 
...
from agents.navigation.routePlanner import RoutePlanner
...
class Agent:
    ...
    "Implements an agent that navigates the scene. This agent respects traffic lights and other vehicles"
    "however it ignores stop signs. It has several functions available to specify the route that the agent"
    "must follow as well as to change its parameters in case a different driving mode is desired" 



    def __init__(self,vehicle, entity, target_speed=30, opt_dict={}, map_inst=None, grp_inst=None, verbose=False, pfa_inst=None):
        ...
        if grp_inst:
            if isinstance(grp_inst, RoutePlanner):
                self._route_planner = grp_inst
            else:
                print("Warning: Ignoring the given map as it is not a 'carla.Map'")
                self._route_planner = RoutePlanner(self._map, self._sampling_resolution,self._vehicle,self.entity,self.verbose, self._pfa)
        else:
            self._route_planner = RoutePlanner(self._map, self._sampling_resolution,self._vehicle,self.entity,self.verbose, self._pfa)
    ...
    def set_destination(self, end_location, start_location=None):
        ...
        route_trace, performance = self.trace_route(start_waypoint, end_waypoint)
